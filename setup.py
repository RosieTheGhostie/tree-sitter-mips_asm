...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_mips_asm", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_mips_asm": ["*.pyi", "py.typed"],
        "tree_sitter_mips_asm.queries": ["*.scm"],
    },
    ext_package="tree_sitter_mips_asm",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_mips_asm/binding.c",
                "src/parser.c",
            ],
            define_macros=[
