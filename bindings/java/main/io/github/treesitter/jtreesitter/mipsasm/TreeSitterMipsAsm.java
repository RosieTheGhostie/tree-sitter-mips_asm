package io.github.treesitter.jtreesitter.mipsasm;

import java.lang.foreign.*;

public final class TreeSitterMipsAsm {
    ...
    private static final TreeSitterMipsAsm INSTANCE = new TreeSitterMipsAsm();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterMipsAsm() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_mips_asm");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-mips-asm");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-mips-asm");
